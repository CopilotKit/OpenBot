    ...
    let (pending, url, user_code) = match provider {
        ...
        "xai" => {
            ...
            let url = body
                ...
                .and_then(|v| v.as_str())
                .map(str::to_owned)
                .unwrap_or(required(&body, "verification_uri")?);
            ...
            let code = loop {
                ...
                match listener.accept() {
                    Ok((mut stream, _)) => {
                        ...
                        if parsed.path() != "/oauth/callback" || query.get("state") != Some(&state)
                        {
...
fn write(
    ...
    loop {
        match std::fs::create_dir(&lock) {
            ...
            Err(error) => return Err(format!("Could not save provider sign-in: {error}")),
        }
    }
    ...
    let unlock = std::fs::remove_dir(lock).map_err(|e| e.to_string());
    result.and(unlock)
}
...
mod tests {
    ...
    fn persisted_credentials_remain_provider_bound() {
        ...
        assert!(!root.join(format!("{FILE}.lock")).exists());
        std::fs::remove_dir_all(root).unwrap();
    }
    #[test]
