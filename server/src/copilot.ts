...
function remoteAgentWithStandingRole(
  ...
  const compose = (tools: GrantedTool[], input: RunAgentInput) => {
    ...
    /*
     * A replay may end with a tool call whose answer never arrived. Remove it before either a
     * browser run or a delegated Channel run reaches the remote endpoint.
     */
...
export function mountCopilotRuntime(
  ...
  const agents = createRequestAgents(identifyActor, resolver) as never;
