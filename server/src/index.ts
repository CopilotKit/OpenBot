...
const actorAgentResolver = createActorAgentResolver({
  ...
  handoffForActor,
  /*
   * What this person has told every coworker of theirs, in every channel.
   *
   * Bound here rather than at one surface, so a routine's headless turn carries the same standing
   * instructions the Bot in front of the person does: a routine is their work done while they are
...
const buildAgentFor = async ({
  ...
  ownerUserId: string;
  agentId: string;
  /** The routine's firing, so the trail says what caused the call rather than only whose it was. */
  ...
  const agent = await actorAgentResolver
    .resolveAgentForActor(actor, agentId, initiator)
    .catch(() => null);
  if (!agent) {
    /*
...
const copilotRuntime = mountCopilotRuntime(
  ...
  identifyUser,
  identifyActor,
  // The default base path, named only because the two seams below follow it.
  undefined,
