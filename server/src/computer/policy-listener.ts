...
export async function startPolicyListener(
  ...
  await connection.listen(ACTION_POLICY_TOPIC, () => {
    /*
     * The payload is ignored on purpose. It says the boundary moved, not what it moved to: a rule
     * list can outgrow NOTIFY's 8000-byte cap, and a server enforcing whatever fitted in a payload
     * would be a subtler version of the bug this fixes. The row is the record; this re-reads it.
     */
