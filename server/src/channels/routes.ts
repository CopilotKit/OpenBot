 ...
 *
 * A malformed cursor is a caller error, not the first page. Falling back used to answer a
 ...
 * 400. A well-formed cursor carrying a non-date `recency` is refused for the same reason
 * it is checked: it would reach `${cursor.recency}::timestamptz` in SQL and answer 500.
 *
 * What is returned is the three fields, rebuilt, so nothing else the caller packed into
