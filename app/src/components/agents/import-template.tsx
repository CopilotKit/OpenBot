...
function ConsentScreen({
  ...
  /*
   * The address box, so the pinned button can send somebody to the field that is keeping it shut.
   * `focus` rather than a scroll call: focusing an off-screen field scrolls it into view in every
   * browser, and it leaves the caret where the person now has to type.
   */
  ...
  return (
    <div className="flex w-full flex-col">
      {/*
       * THE DECISION, PINNED TO THE TOP OF THE PANEL.
       *
       * It used to be the last element in the document, under six sections and the whole of a
       * stranger's instruction set — several thousand pixels of scroll in a 560px panel, with
       * nothing on the first screen to say the control existed at all. Somebody pressed "Use this
       * template", landed here, and could not find a way to say yes.
       *
       * The ordering argument this screen was built on survives the move, because that argument was
       * never about distance. It is that nothing is hidden and nothing is summarised: every word the
       * author wrote is still rendered verbatim below, unabridged, ahead of the capability list. A
       * button at the bottom bought the appearance of a reading gate — one that anybody defeats with
       * a single press of End — and charged a real price for it. So the bar states the cost of
       * pressing it instead, on a line that stays on screen exactly as long as the button does.
       */}
      <header className="sticky top-0 z-10 grid gap-2 border-border border-b bg-sidebar px-8 pt-4 pb-3">
        <Button
        ...
        <div className="flex items-center gap-3">
          <h1 className="min-w-0 flex-1 truncate font-semibold text-lg">
            Import this coworker?
          </h1>
          {/*
           * The label carries a stranger's string, so the button caps its own width rather than
           * growing until the heading beside it has nowhere left to go. This is the one place a cap
           * on model-visible text is safe, and only because it is not the disclosure: the name is
           * rendered in full, unclipped and unwrapped, a few lines below under "What this Bot is".
           *
           * The text is a DIRECT child rather than wrapped in a span for the ellipsis. What reads
           * this button is not only a person — the tests and the recording scripts find it by its
           * label, and a wrapper makes the element they find a span with no `disabled` to read.
           */}
          <Button
            className="max-w-[55%] shrink-0 truncate"
            disabled={
              installing ||
        ...
        {/*
         * One line of state, and which line it is matters more now than it did. An install error
         * used to render beside a button at the very end of the document; with the button up here
         * that error would have appeared thousands of pixels below the press that caused it, on a
        ...
        {installError ? (
          ...
          <div className="flex flex-wrap items-center gap-x-2">
            ...
            {/*
             * The box is in section 3, below however many skills this template carries. A shut
             * button that explains itself without saying where to go is the same complaint one step
             * further in.
             */}
            <Button
              className="px-0 text-xs!"
              onClick={() => addressField.current?.focus()}
              size="sm"
              variant="link"
            >
              Add the address
          ...
          <p className="text-muted-foreground text-xs">
            Importing grants nothing. Everything below is recorded as asked for
            and stays unanswered until somebody decides it.
          </p>
        )}
      </header>

      <div className="flex flex-col gap-7 px-8 pt-7 pb-10">
        {/* 1 ─────────────────────────────────────────────────────────────── */}
        <Section step={1} title="What this Bot is">
          ...
          <div className="grid gap-0.5 rounded-lg border border-border bg-card p-3">
            ...
            {meta.version ? (
              <Claim label="Version" value={meta.version} />
            ) : null}
            {/*
          ...
          {template.notes ? (
            <>
              <p className="text-muted-foreground text-xs">
                A note the author left for you. This one is not given to a
                model.
              </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      {skill.tools.length > 0 ? (
                        <p className="text-muted-foreground text-xs">
                          It names {skill.tools.join(", ")}. Naming a tool is
                          not being given it.
                        </p>
                      ) : null}
                      {resolved?.collides ? (
        ...
        <Section step={3} title="Where it runs">
          {plan.runsOn === "in_process" ? (
            ...
            <>
              <p className="text-sm">
                It runs on this deployment itself, on the model this deployment
                is configured with. There is no address to supply and no other
                host is dialled: nothing leaves your network.
              </p>
              <p className="text-muted-foreground text-sm">
                Nothing had to be arranged for it: the text above under
                &ldquo;What this Bot is&rdquo; becomes its standing
                instructions, and its skills are given to it the same way they
                are to a coworker you built yourself.
              </p>
            </>
          ) : plan.runsOn === "managed_agent" ? (
            <p className="text-sm">
              It runs on the Bot this deployment already runs. Nothing leaves
              your network.
            </p>
          ) : (
            <>
              ...
              <p className="text-sm">
                Every message anyone sends this coworker is sent to this
                address, together with any skill instructions in force.
              </p>
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  /*
                   * Clear of the pinned bar. A browser bringing this field into view has no idea
                   * the top of the scrollport is covered, and one that aligns to the top rather
                   * than to the nearest edge would deliver somebody to a box under the bar.
                   */
              ...
              {hostDiffers ? (
                <p className="rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-sm">
                  ...
                  <span className="font-mono">{claimedHost}</span>. You have
                  typed <span className="font-mono">{typedHost}</span>. Only the
                  address you type is used.
                </p>
              ) : null}
        ...
        <Section step={4} title="What it is asking for">
          {asks === 0 ? (
            ...
            <>
              <p className="text-muted-foreground text-sm">
                Reasons the author wrote. Granting any of these is a separate
                act on a separate screen, by somebody who may.
              </p>
        ...
        <Section step={5} title="What it will be allowed to do">
          ...
          <p className="rounded-md border border-border bg-muted/40 px-3 py-2 text-muted-foreground text-sm">
            This ceiling is applied to this coworker when you import it, and to
            no other. It is enforced by this deployment&rsquo;s own policy
            engine, from rules this deployment compiles — the file never carries
            a rule of its own. Retracting the import takes them away again.
          </p>
          ...
          {policy.isPending ? null : policy.error ? (
            <p className="rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-sm">
              ...
              Only an administrator can see the boundary. Whatever it says
              applies to an imported Bot exactly as it applies to a Bot you
              built yourself.
            </p>
            ...
            <p className="rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-sm">
              ...
              {policy.data.mode === "dry-run" ? (
                <>
                  Its boundary is set to record what it would have refused
                  rather than refuse it, and that setting decides the ceiling
                  above too — nothing in it stops anything until this deployment
                  is switched to stopping actions.{" "}
                </>
              ) : (
                <>
                  Nothing narrows this coworker except the ceiling above.
                  Wherever that ceiling permits something, it can browse, read
                  and write files, and run shell commands exactly like a Bot you
                  built yourself.{" "}
                </>
              )}
              <Link
        ...
        {/*
         * No second Import button down here, deliberately. Two controls with one label is how a
         * person ends up pressing the stale one, and the bar above is on screen at the end of the
         * document exactly as it is at the start.
         */}
      </div>
    </div>
  );
