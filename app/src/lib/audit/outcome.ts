...
export const REFUSED_EVENT_TYPES = [
   ...
   *
   * A refused import leaves nothing behind anywhere else in the product — no Bot, no skill, no
   * grant — so this row is the only place the attempt is counted at all, and counting is the point.
   * One refused paste is somebody's typo. Forty in an afternoon, each turned away for a different
   * reason, is somebody mapping the edges of the parser, and that is only ever visible to a reader
   * who can list them together.
   *
   * `template.capability_requested` is deliberately not here. Nothing was forbidden — configuration
   * travels and capability does not, so an ask that lands unmet is the designed outcome — and
   * drawing it as a refusal would teach a reader to discount the refusals that are real.
   */
